...
- **🚀 开机自动启动(静默)**:勾选后写入 Windows 注册表(`HKCU\...\Run`),开机时**静默启动**——自动隐藏到托盘并开启打印服务,不弹窗。取消勾选即移除。
- **启动面板时自动开启服务**:勾选后,每次打开面板都会自动启动打印服务。

> 托盘功能需要 `pystray` 和 `Pillow`(已在 `requirements.txt` 中,`panel.bat` 会自动安装);
> 若缺失则自动回退为普通最小化。开机自启与静默启动仅 Windows 支持。

#### 命令行参数

```
python control_panel.py --silent      # 静默启动: 隐藏到托盘 + 自动开启服务 (开机自启用的就是它)
